...
theory Variant_dargentisa_CorresProof_Edited
imports "Variant_dargentisa_CorresSetup_Edited"
 ...
 (* XXX: dodgy hack: x_fresh_cogent and uv_fresh_cogent should both be fresh names and must be bound in the definition below.
    This will fail if there is a function called x_fresh_cogent. *) 
       then [@{mk_term "x_fresh_cogent = sint (?tag :: 32 signed word) \<and> uv_fresh_cogent = UFunction (?fun :: string expr) []"
               (tag, fun)} (tag_term, Syntax.read_term @{context} ("Variant_dargentisa_TypeProof." ^ fun_name))]
...
\<close>
ML \<open>
fun typing_tree_of "getVals" = getVals_typing_tree
  | typing_tree_of "putVals" = putVals_typing_tree
...
val HO_call_hints =
     Cogent_functions
     |> Par_List.map (fn f => case CogentHigherOrder.make_HO_call_hints @{context} "variant_dargentisa.c" f of
...
val Cogent_main_tree =
  make_call_tree (Cogent_functions, Cogent_abstract_functions)
    (Symtab.map (K (map (apsnd (map (apsnd (mapBoth (maybe_unprefix "Variant_dargentisa_TypeProof."))))))) HO_call_hints) @{context}
