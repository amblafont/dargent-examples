...
theory Random_seed_CorresProof
imports "Random_seed_CorresSetup"
...
val HO_call_hints =
     Cogent_functions
     |> Par_List.map (fn f => case CogentHigherOrder.make_HO_call_hints @{context} "random_seed.c" f of
...
val Cogent_main_tree =
  make_call_tree (Cogent_functions, Cogent_abstract_functions)
    (Symtab.map (K (map (apsnd (map (apsnd (mapBoth (maybe_unprefix "Random_seed_TypeProof."))))))) HO_call_hints) @{context}
