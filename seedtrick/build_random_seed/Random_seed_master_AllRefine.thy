...
theory Random_seed_master_AllRefine
imports "Random_seed_master_NormalProof"
"Random_seed_master_SCorres_Normal"
"Random_seed_master_CorresProof"
"Random_seed_master_MonoProof"
 ...
 *)
local_setup \<open>
let val typeproof_thy = "Random_seed_master_TypeProof"
in
...
locale Random_seed_master_cogent_shallow =
  "random_seed_master_pp_inferred" + correspondence +
...
sublocale Random_seed_master_cogent_shallow \<subseteq> Random_seed_master _ upd_abs_typing abs_repr
  by (unfold_locales)

sublocale Random_seed_master_cogent_shallow \<subseteq> shallow val_abs_typing
  by (unfold_locales)

sublocale Random_seed_master_cogent_shallow \<subseteq> correspondence_init
  by (unfold_locales)


context Random_seed_master_cogent_shallow begin

(* Generate end-to-end refinement theorems, exported to corres_shallow_C_f *)
local_setup \<open>
filter (member op= Cogent_functions) entry_func_names
|> fold (fn f => fn lthy => let
     val thm = make_corres_shallow_C "Random_seed_master_Shallow_Desugar" "Random_seed_master_TypeProof" lthy f
